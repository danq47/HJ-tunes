...
C       pt_bins1(66)=300d0
      ...
      do ixx=1,60
         pt_bins1(ixx)=(ixx-1.0)*5d0
      enddo

      do ixx=1,200
         pt_bins2(ixx)=(ixx-1.0)*2d0
      enddo
...
C       y_bins1(28)=5d0
      ...
      do jxx=1,300
         if(pt_bins1(jxx).gt.-1d5) len_pt1=len_pt1 + 1
         if(pt_bins2(jxx).gt.-1d5) len_pt2=len_pt2 + 1
...
!      
!     pt-j1 and pt-H cut into y_H bins
      
      do kxx=1,len_y
         
         negative = .false.
         if(y_bins1(kxx).lt.-1d-2) negative = .true.
         ...
         if(kxx.eq.1) then      ! initial cut - take from Y-higgs=-inft up to the lowest bin edge
            call bookupeqbins('pth-5GeV-'//'minus-inf-yH-'//tmp,5d0,0d0,300d0)
            call bookupeqbins('pth-2GeV-'//'minus-inf-yH-'//tmp,2d0,0d0,400d0)
            tmp2=tmp
         elseif(kxx.gt.1.and.kxx.lt.len_y) then ! This includes all the different cuts in the middle
            call bookupeqbins('pth-5GeV-'//s1(kxx)(1:ls1),5d0,0d0,300d0)
            call bookupeqbins('pth-2GeV-'//s1(kxx)(1:ls1),2d0,0d0,400d0)
            tmp2=tmp
         elseif(kxx.eq.len_y) then ! and this cuts Y-higgs > highest bin edge
            call bookupeqbins('pth-5GeV-'//tmp2(1:t2)//'-yH-inf',5d0,0d0,300d0)
            call bookupeqbins('pth-2GeV-'//tmp2(1:t2)//'-yH-inf',2d0,0d0,400d0)
         endif
